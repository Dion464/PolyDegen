...
const AdminResolution = () => {
  ...
  const handleResolve = async (marketId, outcome) => {
    ...
    try {
      ...
      // NOTE: We don't auto-pay winners anymore - they claim via notification button
      // This allows users to click "Claim" in their notification to receive their winnings
      const winnersCount = participants.filter(p => {
      ...
      if (winnersCount > 0) {
        showGlassToast({ 
          title: `${winnersCount} winners can now claim their winnings!`, 
          icon: '🎉' 
        });
      }
